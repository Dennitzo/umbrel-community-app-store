...
export default function Addresses() {
  ...
  const safeTopAddresses = topAddresses ?? { ranking: [] };
  ...
  const calculateSum = (top: number) =>
    safeTopAddresses.ranking.slice(0, top).reduce((acc, curr) => acc + curr.amount, 0);
    ...
    <>
      ...
      <div className="flex w-full flex-col rounded-4xl bg-white p-4 text-left text-gray-500 sm:p-8">
        <PageTable
          ...
          rows={safeTopAddresses.ranking.slice(0, 100).map((addressInfo) => [
            addressInfo.rank + 1,
